...
void SocketManager::Delete(int pos)
{
        ...
        vector<bool>::iterator iter6 = socket_error.begin();
        for (int i = 0;i < pos;iter6++, i++) ;
        socket_error.erase(iter6);
}
//-----------------------------------------------------------------------------
...
void SocketManager::Connect(int i)
{
        ...
        if (sock_types[i] == CALAOS)
        {
                //try to login
                string cmd = "login ";
                cmd += Utils::get_config_option("calaos_user") + " ";
                cmd += Utils::get_config_option("calaos_password");
                cmd += "\r\n";
                if (socket->Send(cmd))
                {
                        string res;

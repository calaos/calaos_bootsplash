...
-(void)setPages:(NSMutableArray *)pages {
	if (pages != nil) {
	    for(int i=0;i<[_pages count];i++) 
            {
                    UIViewController *view = [ _pages objectAtIndex: i ];
...
- (void)layoutViews {
	if ([ _pages count ] <= 3) {
		for(int i=0;i<[ _pages count];i++) {
                        UIViewController *controller = [ _pages objectAtIndex: i ];
	...
	if (pageNum == 0) {
		for(int i=0;i<3;i++) {
			...
			page.hidden = NO;
                ...
                UIViewController *controller = [ _pages objectAtIndex: 3 ];
                page = controller.view;
		page.hidden = YES;
		_zeroPage = 0;
	}
	...
	/* Right boundary */
		...
		for(int i=pageNum-2;i<=pageNum;i++) {
			...
			page.hidden = NO;
                ...
                UIViewController *controller = [ _pages objectAtIndex: [ _pages count ]-3 ];
                UIView *page = controller.view;
		page.hidden = YES;
		_zeroPage = pageNum - 2;
	}
